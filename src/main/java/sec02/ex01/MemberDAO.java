	...
	public List<MemberVO> listMembers()
	{
		...
		try {
			...
			while (rs.next())
			{
				...
				MemberVO memberVO 
					= new MemberVO(id, pwd, name, email, joinDate); //조회한 회원 정보를 레코드별로 MemberVO 객체의 속성에 저장
				membersList.add(memberVO); //membersList에 MemberVO 객체들을 차례대로 저장
			}
			rs.close();
			pstmt.close();
			conn.close();
